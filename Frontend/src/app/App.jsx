...
const generateMockTurn = (prompt) => {
  const isCode = prompt.toLowerCase().includes('code') || prompt.toLowerCase().includes('write') || prompt.toLowerCase().includes('implement') || prompt.toLowerCase().includes('function') || prompt.toLowerCase().includes('javascript') || prompt.toLowerCase().includes('python');
...
export default function App() {
  ...
  const handleAddFollowUpTurn = useCallback(() => {
    ...
    setHistory(prev => prev.map(item => item.id === battleData.id ? updatedSession : item));
    setView('battle');
  }, [battleData, prompt]);
  ...
  return (
    <>
      ...
      {/* Loading Overlay */}
      {view === 'loading' && (
        <LoadingScreen onComplete={battleData ? handleAddFollowUpTurn : handleLoadingComplete} />
      )}
